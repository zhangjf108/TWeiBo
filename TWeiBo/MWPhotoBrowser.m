...
- (void)viewDidLoad {
    ...
    // Navigation bar
    navigationBar = [[UINavigationBar alloc] initWithFrame:[self frameForNavigationBarAtOrientation:self.interfaceOrientation]];
    navigationBar.tintColor = nil;
    navigationBar.barStyle = UIBarStyleBlackTranslucent;
    navigationBar.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleWidth;
    
    UINavigationItem *item = [[UINavigationItem alloc] initWithTitle:@"查看原图"];
    item.leftBarButtonItem = BARBUTTON(@"返回", @selector(back:));
    
    if (isSave) {
        item.rightBarButtonItem = BARBUTTON(@"保存", @selector(saveImage:));
    }
...
- (IBAction) saveImage:(id)sender {
    MWPhoto *mwPhoto = [photos objectAtIndex:currentPageIndex];
    //保存到相册
    UIImageWriteToSavedPhotosAlbum([mwPhoto image], nil, nil, nil);
    [OtherUtil alertWithTitle:nil message:@"保存成功" cancel:@"确定"];
}
...
- (void)performLayout {
    ...
    navigationBar.frame = CGRectMake(0, 20, frame.size.width, frame.size.height);
...
- (void)setControlsHidden:(BOOL)hidden {
	...
	[navigationBar setAlpha:hidden ? 0 : 1];
